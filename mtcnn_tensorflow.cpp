...
int MtcnnTensorflow::Init(const std::string& model_file) {
...
int MtcnnTensorflow::PNet(cv::Mat& img, const ScaleWindow& win,
...
int MtcnnTensorflow::RNet(cv::Mat& img, std::vector<FaceBox>& pnet_boxes,
...
int MtcnnTensorflow::ONet(cv::Mat& img, std::vector<FaceBox>& rnet_boxes,
                   std::vector<FaceBox>* output_boxes) {
...
void MtcnnTensorflow::Detect(cv::Mat& src, std::vector<FaceBox>* face_boxes) {
...
void MtcnnTensorflow::PyramidScales(int height, int width, int min_size, float factor,
                            std::vector<ScaleWindow>* scale_windows) {
...
void MtcnnTensorflow::ProcessBoxes(std::vector<FaceBox>& input, int img_h, int img_w,
...
void MtcnnTensorflow::Normalize(std::vector<FaceBox>& input, float threshold, int type,
...
void MtcnnTensorflow::Regress(std::vector<FaceBox>* boxes) {
  float h = 0.0f;
  float w = 0.0f;
...
void MtcnnTensorflow::Square(std::vector<FaceBox>* boxes) {
  float h = 0.0f;
  float w = 0.0f;
  float l = 0.0f;
  for (FaceBox& box : *boxes) {
...
void MtcnnTensorflow::Padding(int img_h, int img_w, std::vector<FaceBox>* boxes) {
  for (FaceBox& box : *boxes) {
...
void MtcnnTensorflow::Patch(const cv::Mat& img, FaceBox& box, float* data_to,
...
void MtcnnTensorflow::GenerateBoundingBox(const float* confidence_data,
